...
const AllApplicationStatus = ({
  ...
  const handleAddInvoice = async () => {
    if (!downPayment) {
      return toast.error("Down payment is required");
    }
    setLoading(true);
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/post-application`,
        {
          application_id: selectedForInvoice.id,
      ...
      <div className="bg-gray-100 w-[95%] max-w-5xl rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5">
          {!selectedForInvoice ? (
            ...
            <>
              {loading && (
                <div className="absolute top-0 left-0 w-full flex  justify-center h-full bg-white/40 py-10">
                  <div className="animate-spin mt-50 rounded-full h-12 w-12 border-b-2 border-gray-900"></div>
                </div>
              )}
                ...
                "approved" ? (
                  <div className="space-y-6">
                    ...
                    <div className="flex justify-end">
                      <button
                        ...
                        disabled={loading}
                        className={`bg-[#4caf50] text-white px-8 py-2.5 rounded-lg font-bold hover:bg-green-600 uppercase tracking-wide ${loading ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                      >
                        Add to Invoice
                      </button>
                    </div>
                  ...
                  <div className="mt-10 pt-6 border-t border-gray-100">
                    <p className="text-gray-400">
                      You are unable to submit the order and invoice under
                      finance because the application status is not marked as
                      "fulfilled".
                    </p>
